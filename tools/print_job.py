...
def parse_args(argv):
    ...
    if args['attempt_str'] is None:
        if args['reqnum'] is None:
            ...
            args['attempt_str'] = f"{args['unitname']}_r{args['reqnum']}p{int(args['attnum']):02d}"
...
def print_single_wrap(wrapnum, numwraps, expnumwrap, jdict, jwdict, wdict, indent='\t'):
    ...
    print(f"{indent}job: {pfwutils.pad_jobnum(jdict['jobnum'])} (jk={jdict['jobkeys']})  {numwraps:d}/{expnumwrap:d}  {jstate} - {jstatus}   wrap: {wrapnum} {modname} (wk={wrapkeys}) - {state} {status}")
    ...
    for run in args['runs']:
        ...
        if attinfo is None:
            ...
            if 'endtime' in attinfo and attinfo['endtime'] is not None:
                print(f"Note:  run has finished with status {attinfo['status']}")
            ...
            jwrap_byjob, _ = pfwutils.index_jobwrapper_info(jobwrapinfo)
            ...
            for blknum, blkval in blockinfo.items():
                print_single_block(blknum, blkval, job_byblk, jwrap_byjob, wrap_byjob, args['verbose'])
